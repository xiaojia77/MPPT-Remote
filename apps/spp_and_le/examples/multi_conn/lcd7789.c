...
void Lcd_printf24x24(uint8_t x,uint8_t y,uint8_t *str)  //must be string
{
...
{
   uint8_t i;
   for(i=1;i<=7;i++)Lcd_Clear24x24(5,30*i);

    //限制下标长度
   if(!MenuData.index[MenuData.current_id] ) MenuData.index[MenuData.current_id] = 1 ; 
   if(MenuData.index[MenuData.current_id] > 7 ) MenuData.index[MenuData.current_id] = 7 ;
...
void menu_select(uint8_t key)
{
    ...
    {
        case 0: // 上
            if( MenuData.index[MenuData.current_id] > 1)MenuData.index[MenuData.current_id]--;
            else MenuData.index[MenuData.current_id] = Menu_Tab[MenuData.current_id].max_menu_num;
            menu_select_display();
            break;
        case 1: // 下
            if( MenuData.index[MenuData.current_id] < Menu_Tab[MenuData.current_id].max_menu_num )MenuData.index[MenuData.current_id]++;
            else MenuData.index[MenuData.current_id] = 1;
            menu_select_display();
            break;
        case 2: // ←
            MenuData.current_id =  Menu_Tab[MenuData.current_id].up_id;
              if( Menu_Tab[MenuData.current_id].display_operation != NULL)Menu_Tab[MenuData.current_id].display_operation();
            break;
    }
}
...
void main_menu() //主菜单
{
    ...
    Lcd_printf24x24(30,120,"蓝牙指定连接");
    Lcd_printf24x24(30,150,"蓝牙自动连接");
    Lcd_printf24x24(30,180,"版本号查询");
   // Lcd_printf24x24(30,210,"count ");
    Lcd_printf24x24(5,30*MenuData.index[MenuData.current_id],"＞");
}
...
void IRorBL_set_menu()
{
    ...
    if(roter_data.Ble_Onflag)Lcd_printf24x24(30,30,"蓝牙通信：开");
    else Lcd_printf24x24(30,30,"蓝牙通信：关");
    if(roter_data.Ir_Onflag)Lcd_printf24x24(30,60,"红外通信：开");
    else Lcd_printf24x24(30,60,"红外通信：关");
    Lcd_printf24x24(5,30*MenuData.index[MenuData.current_id],"＞");
}
...
void IRorBL_set_operation(uint8_t key)
{
        ...
        switch (key)
        {
            ...
            case 3: // →
                ...
                {
                    case 1:
                        if(roter_data.Ble_Onflag)roter_data.Ble_Onflag = 0;
                        else roter_data.Ble_Onflag = 1;
                        if(roter_data.Ble_Onflag)Lcd_printf24x24(30,30,"蓝牙通信：开");
                        else Lcd_printf24x24(30,30,"蓝牙通信：关");
                        break;
                    case 2:
                        if(roter_data.Ir_Onflag)roter_data.Ir_Onflag = 0;
                        else roter_data.Ir_Onflag = 1;
                        if(roter_data.Ir_Onflag)Lcd_printf24x24(30,60,"红外通信：开");
                        else Lcd_printf24x24(30,60,"红外通信：关");
                        break;
                }
...
{
    Lcd_Clear(BLACK);
    Lcd_printf24x24(120-24*3,0,"版本号查询");
}
