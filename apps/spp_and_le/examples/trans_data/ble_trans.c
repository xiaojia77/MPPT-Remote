...
static u8  trans_adv_data[ADV_RSP_PACKET_MAX];//max is 31          广播包
static u8  trans_scan_rsp_data[ADV_RSP_PACKET_MAX];//max is 31     回应包
...
const gatt_server_cfg_t trans_server_init_cfg =  //读写回调
{
...
#else
    .client_config = NULL, //client not use
#endif
 ...
 */
/*************************************************************************************************/
static int trans_event_packet_handler(int event, u8 *packet, u16 size, u8 *ext_param)   //GATT返回
{
    /* log_info("event: %02x,size= %d\n",event,size); */

    switch (event) {

    case GATT_COMM_EVENT_CAN_SEND_NOW: //协议栈初始化完毕
#if TEST_AUDIO_DATA_UPLOAD
        trans_test_send_audio_data(0);
#endif
        break;

    case GATT_COMM_EVENT_SERVER_INDICATION_COMPLETE:  //应答完成
        log_info("INDICATION_COMPLETE:con_handle= %04x,att_handle= %04x\n", \
                 little_endian_read_16(packet, 0), little_endian_read_16(packet, 2));
        break;


    case GATT_COMM_EVENT_CONNECTION_COMPLETE:  //链接完毕 
    ...
    case GATT_COMM_EVENT_DISCONNECT_COMPLETE:  //bl 断开
    ...
    case GATT_COMM_EVENT_ENCRYPTION_CHANGE:  //加密完成
    ...
    case GATT_COMM_EVENT_CONNECTION_UPDATE_COMPLETE:  //更新完毕 
...
static int trans_make_set_adv_data(void)  //广播数据
{
    u8 offset = 0;
...
#else
    offset += make_eir_packet_val(&buf[offset], offset, HCI_EIR_DATATYPE_FLAGS, FLAGS_GENERAL_DISCOVERABLE_MODE | FLAGS_EDR_NOT_SUPPORTED, 1); //一般模式 EDR不支持
#endif

    offset += make_eir_packet_val(&buf[offset], offset, HCI_EIR_DATATYPE_COMPLETE_16BIT_SERVICE_UUIDS, 0xAF30, 2); //16bit uid

    char *gap_name = ble_comm_get_gap_name(); 
    u8 name_len = strlen(gap_name);
    u8 vaild_len = ADV_RSP_PACKET_MAX - (offset + 2);
    if (name_len < vaild_len) //长度合法
    {
        offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_COMPLETE_LOCAL_NAME, (void *)gap_name, name_len); //名字
        adv_name_ok = 1;
    } 
    else 
    {
    ...
    trans_server_adv_config.adv_data_len = offset;   //注册信息
 ...
 */
/*************************************************************************************************/
static int trans_make_set_rsp_data(void)  //响应包
{
    u8 offset = 0;
    ...
    if (!adv_name_ok)  //广播正常发送
    {
        char *gap_name = ble_comm_get_gap_name(); //gap 名字
        u8 name_len = strlen(gap_name);
        u8 vaild_len = ADV_RSP_PACKET_MAX - (offset + 2);
