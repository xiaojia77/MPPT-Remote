...
//---------------------------------------------------------------------
void central_set_soft_poweroff(void) //软件关机
{
    ...
    btstack_ble_exit(0);
    //延时300ms，确保BT退出链路断开
    sys_timeout_add(NULL, power_set_soft_poweroff, WAIT_DISCONN_TIME_MS);  //os—delay
#else
    power_set_soft_poweroff();
#endif
}

static void central_app_start()     //初始化
{
    log_info("=======================================");
...
static int central_state_machine(struct application *app, enum app_state state, struct intent *it) //状态机
{
    switch (state) {
    case APP_STA_CREATE:
        break;
...
static int central_bt_hci_event_handler(struct bt_event *bt) //HCI事件
{
...
static int central_bt_connction_status_event_handler(struct bt_event *bt) //连接status状态事件改变
{
...
extern void cetl_disconnect(void); //断开连接

static void central_key_event_handler(struct sys_event *event) //按键事件
{
    /* u16 cpi = 0; */
    u8 event_type = 0;
    u8 key_value = 0;
...
static int central_event_handler(struct application *app, struct sys_event *event)
{
    switch (event->type) {
    case SYS_KEY_EVENT:
        central_key_event_handler(event); //按键事件  软关机 和 测试断开连接
        return 0;

    case SYS_BT_EVENT:  //蓝牙层
#if (TCFG_USER_BLE_ENABLE)
