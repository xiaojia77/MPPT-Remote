...
//common
u32  ble_comm_cbuffer_vaild_len(u16 conn_handle);                   //检测长度是否合法
int ble_comm_att_send_data(u16 conn_handle, u16 att_handle, u8 *data, u16 len, att_op_type_e op_type); //ATT发送数据
bool ble_comm_att_check_send(u16 conn_handle, u16 pre_send_len);    //检测是否可以发送
...
s8 ble_comm_dev_get_index(u16 handle, u8 role);   //获取handle的 index
s8 ble_comm_dev_get_idle_index(u8 role);          //获取可分配的 index
u8 ble_comm_dev_get_handle_role(u16 handle);      //获取handle的角色
u16 ble_comm_dev_get_handle(u8 index, u8 role);   //通过下标获取对应的handle
void ble_comm_set_config_name(const char *name_p, u8 add_ext_name);  //配置GATT的名字
void ble_comm_init(const gatt_ctrl_t *control_blk); //GATT初始化
void ble_comm_exit(void);                //GATT删除 在蓝牙栈退出前使用
void ble_comm_module_enable(u8 en);      //GATT使能
int ble_comm_set_connection_data_length(u16 conn_handle, u16 tx_octets, u16 tx_time);
int ble_comm_set_connection_data_phy(u16 conn_handle, u8 tx_phy, u8 rx_phy, u16 phy_options); //配置物理链路带宽
...
int ble_gatt_server_characteristic_ccc_set(u16 conn_handle, u16 att_ccc_handle, u16 ccc_config); //更新 notify or indicate 通知功能使能
u16 ble_gatt_server_characteristic_ccc_get(u16 conn_handle, u16 att_ccc_handle);//获取 notify or indicate 通知功能使能
void ble_gatt_server_set_update_send(u16 conn_handle, u16 att_handle, u8 att_handle_type); //ota 设置OTA升级使用的 连接handle 和 att handle
void ble_gatt_server_receive_update_data(void *priv, void *buf, u16 len); //ota 数据处理
